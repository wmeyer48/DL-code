...
uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms, Vcl.Buttons, Vcl.Dialogs, Vcl.StdCtrls,
  Vcl.ExtCtrls, Vcl.ComCtrls, Winapi.CommCtrl, Winapi.ShellAPI, Vcl.Clipbrd, Vcl.Menus, Vcl.FileCtrl, System.Win.Registry, LsConsts, Winapi.ShlObj,
  Winapi.ActiveX, System.Win.ComObj, Vcl.ImgList, LsTheme;
...
Function DeleteNonNumberChars(Const InString: String): string;  //882>
  ...
  begin
    ...
    begin
      Ch := InString[i];
      if (ch in ['0'..'9', '-', '+']) or (Ch = FormatSettings.DecimalSeparator) then
      begin
        S := S + Ch;
      end;
      i := i + 1;
...
constructor TLsDirTreeCombo28.Create(AOwner: TComponent);
var
  sfi: TShFileInfo;
...
constructor TLsDirTree21.Create(AOwner: TComponent);
var
  sfi: TShFileInfo;
...
begin
  if Value <> FTimeFormat then
    FTimeFormat := Value;
  case FTimeFormat of
    tfLongFormat :  FTFormatStr := FormatSettings.LongTimeFormat;
    tfShortFormat:  FTFormatStr := FormatSettings.ShortTimeFormat;
    tfCustomize  :
      begin
        DefaultLCID := GetThreadLocale;
        FTFormatStr := '';
...
constructor TNetPathDlg.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  FDrvLetter := '';
  FNetPath := '';
